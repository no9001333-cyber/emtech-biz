...
# 너무 임박하면 준비할 시간이 없고, 너무 멀면 아직 실익이 없어서 기본값을 7~30일로 뒀었는데,
# 대시보드에서 오늘로부터 30일보다 뒤 날짜로 검색해도 아무것도 안 나온다는 문의가 있어
# 60일로 넓혔습니다 (수집 자체가 30일 제한 때문에 안 되고 있었던 것이라, 대시보드 검색창
# 자체의 버그는 아니었습니다). 더 멀리까지 보고 싶으면 이 숫자를 더 늘리면 됩니다.
# MIN_DAYS_UNTIL_DEADLINE도 7일에서 1일로 낮췄습니다 — 경쟁 서비스(대박낙찰정보)의
# 맞춤입찰정보와 대조해보니, 마감이 1~3일 앞으로 임박한 학교 통신공사류 공고들이
# 게시 즉시 이 7일 기준에 걸려 한 번도 수집되지 못하고 있었습니다(한 번 걸러지면
# bids.json에 아예 안 들어가서 나중에 마감이 임박해도 영영 안 보임). 사용자가 1일로
# 낮추기로 결정했습니다.
# 2026-08-24: 1일 기준도 여전히 사각지대였습니다 - dbsbid와 대조해보니 "[긴급]" 소액
# 공고처럼 투찰마감이 당일(24시간 이내)인 공고들이 이 1일 기준에 걸려 계속 누락되고
# 있었습니다. 파이프라인이 하루라도 못 돌면(러너 장애 등) 그 사이 올라온 당일마감
# 공고는 재개 시점엔 이미 기준 미달이라 영영 안 보이는 문제까지 겹쳤습니다. 사용자가
# "누락되지 않도록"을 최우선으로 하기로 해서 0일로 낮췄습니다 - 아직 마감 전이기만
# 하면(과거 공고는 여전히 제외) 목록에 남습니다.
# 마감이 지난 뒤에도 개찰 결과(낙찰자/낙찰금액)를 확인할 수 있도록,
# 마감 후 POST_DEADLINE_TRACK_DAYS일까지는 목록에 계속 남겨둠.
MIN_DAYS_UNTIL_DEADLINE = 0
MAX_DAYS_UNTIL_DEADLINE = 60
...
AWARDS_HTML_PATH = os.path.join(DOCS_DIR, "awards.html")
