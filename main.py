...
- 나라장터 / LH / 국방전자조달(D2B) / 한국수자원공사 / 한국전력공사 / 한국가스공사
  입찰공고 수집기와, 나라장터·LH 낙찰정보(개찰결과) 수집기를 모두 돌립니다
  (2026-09-18: LH 낙찰정보 추가 - dbsbid 대조 결과 LH 낙찰결과가 전혀 없어서
  경쟁사 낙찰가·사정율을 못 보여주던 문제를 보완).
...
- 투찰마감이 지난 공고는(나라장터·LH 모두), 같은 소스·공고번호로 낙찰정보가
  있으면 그 결과(낙찰자/낙찰금액/사정율)를 공고 데이터에 그대로 붙여서 저장합니다.
...
def main():
    ...
    try:
        from scrapers.g2b_awards_probe import probe_missing_awards
        probe_missing_awards()
    except Exception as e:
        print(f"[낙찰정보 진단] 건너뜀: {e}")
    ...
    # 2026-09-18: 예전엔 나라장터만 지원했다. LH 낙찰결과가 없어서 dbsbid와
    # 대조했을 때 경쟁사 낙찰가·사정율을 전혀 못 보여주는 문제가 있었다.
    new_awards = _run_source("나라장터 낙찰정보", bool(G2B_AWARDS_SERVICE_KEY), fetch_g2b_awards, status_list)
    new_awards += _run_source("LH 낙찰정보", bool(LH_SERVICE_KEY), fetch_lh_awards, status_list)
    awards_by_notice = {}
    for award in new_awards:
        award["collected_at"] = now_str
        # 이월/매칭 키는 bid 쪽 _dedupe_key()와 반드시 같은 형식(source::notice_no)
        # 이어야 한다 - 예전엔 "나라장터::"로 고정돼 있어서 LH 낙찰정보를 추가해도
    ...
    # 3) 마감이 지난 공고에(나라장터·LH 모두), 같은 소스·공고번호의 낙찰정보가 있으면 결과를 붙여줌
    matched = 0
    for bid in kept:
